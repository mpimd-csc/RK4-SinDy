...
class parameters:
    ...
    num_iter = 9
    lr: float = 5e-3
    save_model_path: str = './Results/MMkinectics/'
...
x = x + np.random.rand(*x.shape)*np.max(x)*Params.noiselevel
...
# Define dataloaders
train_dset = list(zip(torch.tensor(x).float(),Ts))
...
if Params.tikz_save:
    tikzplotlib.save(Params.save_model_path + "MMKinectics_Pareto.tex")
plt.show()
fig.savefig(Params.save_model_path + "MMKinectics_Pareto.pdf", bbox_inches = 'tight',pad_inches = 0)
...
x_learnt = np.transpose(sol_learnt.y).reshape(1,-1,Params.dim_x)*data_nor.std + data_nor.mean

    
xp0 = np.linspace(0.25,4,num = 20)
TrainingRegime = True
...
if Params.tikz_save:
    tikzplotlib.save(Params.save_model_path + "MMKinectics_simulation.tex")
plt.show()
fig.savefig(Params.save_model_path + "MMKinectics_simulation.pdf", bbox_inches = 'tight',pad_inches = 0)
